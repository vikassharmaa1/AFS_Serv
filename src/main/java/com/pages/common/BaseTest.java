...
public class BaseTest {
	...
	public WebDriverWait seleniumWait(int seconds) {
		...
		return SeleniumFactory.getInstance().webDriveWait(selenium(), seconds);
	}
	...
	final public void oneTimeBaseTearDown(ITestResult result) throws Exception {
		...
		if (result.getStatus() == ITestResult.FAILURE) {
			...
			try {
				String screenshotPath = TakeScreenShot.capture(selenium(), result.getName());
			} catch (Exception e) {
				
				e.printStackTrace();
			}
			SeleniumFactory.test.get().fail(result.getThrowable());
			String screenshotPath = "./" + screenshotPath.substring(screenshotPath.indexOf("screenshots"));
			System.out.println("Screenshot location is - " + screenshotPath);
			SeleniumFactory.test.get().fail("Attached Snapshot for the failure"
	...
	public Object[][] dataProviderExcel(Method m) throws Exception {
